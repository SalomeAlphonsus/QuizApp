...
const WorkPlaceForm = () => {
  ...
  const workTypes = [
    { id: 1, label: "School", color: "bg-app-blue", icon: <FaUser className="w-6 h-6 text-white" /> },
    { id: 2, label: "Higher Education", color: "bg-app-orange", icon: <FaChalkboardTeacher className="w-6 h-6 text-white" /> },
    { id: 3, label: "Teams", color: "bg-app-green", icon: <FaGraduationCap className="w-6 h-6 text-white" /> },
    { id: 4, label: "Business", color: "bg-app-red", icon: <FaBriefcase className="w-6 h-6 text-white" /> },
  ];
  ...
  const [errorMsg, setErrorMsg] = useState('');
    ...
    const handleSubmit = () =>{
      if (workType =='') {
        setErrorMsg('select a work type') 
      } else{
        router.push(`/profileAccount?accountType=${accountType}&workType=${workType}`)
      }
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
      ...
      <progress className="progress progress-primary w-56 mt-2" value="40" max="100"></progress>
      </div>
      {/* Heading */}
      ...
      <div className="w-full max-w-md space-y-4">
        {workTypes.map((type) => (
          <button
            key={type.id}
            className={`flex items-center p-4 ${workType === type.label ? "bg-blue-500" : "bg-white"} rounded-lg shadow-md hover:bg-gray-100 w-full`}
            onClick={() => setWorkType(type.label)}
          >
